"""
Enhanced Arris Status Client with HTTP Compatibility and Built-in Quick Connectivity Check
==========================================================================================
...
comprehensive performance instrumentation, and intelligent connectivity checking.

This client includes:
- Arris HTTP compatibility handling to work around urllib3's strict HTTP parsing
- Built-in quick connectivity check for fast failure on unreachable devices
- Automatic local network detection with optimized timeouts
...
- Quick connectivity check for fast failure (2s vs 10s+ for unreachable devices)
- Automatic local network detection and timeout optimization
...
class PerformanceInstrumentation:
    ...
    - Quick connectivity check timing
    """
    ...
    def get_performance_summary(self) -> Dict[str, Any]:
        ...
        # Quick connectivity check metrics
        quick_check_metrics = [m for m in self.timing_metrics if "quick_connectivity" in m.operation.lower()]
        quick_check_overhead = sum(m.duration for m in quick_check_metrics)
        ...
        return {
            "session_metrics": {
                ...
                "quick_check_overhead": quick_check_overhead
            },
    ...
    def _generate_performance_insights(self, operation_stats: Dict[str, Any], total_time: float) -> List[str]:
        ...
        # Quick connectivity check performance
        quick_ops = [op for op in operation_stats.keys() if "quick_connectivity" in op.lower()]
        if quick_ops:
            quick_time = sum(operation_stats[op]["avg_time"] for op in quick_ops)
            if quick_time > 1.0:
                insights.append(f"Quick connectivity check taking {quick_time:.2f}s - may need timeout adjustment")
            else:
                insights.append(f"Quick connectivity check performing well: {quick_time:.2f}s")
...
class ArrisStatusClient:
    """
    Enhanced Arris modem client with HTTP compatibility, performance instrumentation,
    and built-in quick connectivity checking.

    This client provides high-performance access to Arris cable modem status
    with built-in HTTP compatibility handling for urllib3 parsing strictness,
    comprehensive performance monitoring, and intelligent connectivity validation.
    ...
    def __init__(
        ...
        # NEW: Quick connectivity options
        quick_check: bool = True,      # Enable quick connectivity check
        fail_fast: bool = True,        # Fail fast for unreachable local devices
        quick_timeout: float = 2.0     # Timeout for quick connectivity check
    ):
        """
        Initialize the Arris modem client with HTTP compatibility, instrumentation,
        and optional quick connectivity checking.
        ...
        Args:
            ...
            quick_check: Enable quick TCP connectivity check before HTTPS attempts (default: True)
            fail_fast: Fail quickly for unreachable devices (especially local network) (default: True)
            quick_timeout: Timeout for quick connectivity check in seconds (default: 2.0)
        """
        self.host = host
        self.port = port
        ...
        # Auto-detect if this looks like a local network device
        self._is_local_network = self._detect_local_network(host)

        # Adjust timeouts for local vs remote devices
        if self._is_local_network and timeout == (3, 12):  # Using default timeout
            self.timeout = (2, 8)  # Shorter timeouts for local devices
            logger.debug(f"🏠 Local network detected, using shorter timeouts: {self.timeout}")
        else:
        ...
        device_type = "local" if self._is_local_network else "remote"
        logger.info(f"🛡️ ArrisStatusClient v1.3 with HTTP compatibility initialized for {host}:{port} ({device_type})")
        logger.info(f"🔧 Mode: {mode_str}, Workers: {self.max_workers}, Retries: {max_retries}")
        logger.info(f"⚡ Quick check: {'enabled' if quick_check else 'disabled'}, Fail fast: {'enabled' if fail_fast else 'disabled'}")
        if enable_instrumentation:
            logger.info("📊 Performance instrumentation enabled")

    def _detect_local_network(self, host: str) -> bool:
        """Detect if the host appears to be on the local network."""
        return (
    ...
    def _quick_connectivity_check(self) -> Tuple[bool, Optional[str]]:
        """
        ...
        if not self.quick_check:
            return True, None  # Skip check if disabled

        start_time = self.instrumentation.start_timer("quick_connectivity_check") if self.instrumentation else time.time()

        try:
            logger.debug(f"🔍 Quick connectivity check: {self.host}:{self.port} (timeout: {self.quick_timeout}s)")

            with socket.create_connection((self.host, self.port), timeout=self.quick_timeout):
                logger.debug(f"✅ TCP connection successful")

                if self.instrumentation:
                    self.instrumentation.record_timing("quick_connectivity_check", start_time, success=True)

                return True, None

        except socket.timeout:
            error_msg = f"Connection timeout - {self.host}:{self.port} not responding within {self.quick_timeout}s"
            if self._is_local_network:
                error_msg += " (local device should respond quickly)"

            if self.instrumentation:
                self.instrumentation.record_timing("quick_connectivity_check", start_time, success=False, error_type="timeout")

            return False, error_msg

        except socket.gaierror as e:
            error_msg = f"DNS resolution failed for {self.host}: {e}"

            if self.instrumentation:
                self.instrumentation.record_timing("quick_connectivity_check", start_time, success=False, error_type="dns_error")

            return False, error_msg

        except ConnectionRefusedError:
            error_msg = f"Connection refused - {self.host}:{self.port} not accepting connections"

            if self.instrumentation:
                self.instrumentation.record_timing("quick_connectivity_check", start_time, success=False, error_type="connection_refused")

            return False, error_msg

        except OSError as e:
            error_msg = f"Network error connecting to {self.host}:{self.port}: {e}"

            if self.instrumentation:
                self.instrumentation.record_timing("quick_connectivity_check", start_time, success=False, error_type="network_error")
    ...
    def authenticate(self) -> bool:
        """Perform HNAP authentication with optional quick connectivity check and HTTP compatibility."""
        # NEW: Quick connectivity check before attempting authentication
        if self.quick_check:
            is_reachable, error_msg = self._quick_connectivity_check()

            if not is_reachable:
                if self.fail_fast:
                    # Fail immediately for unreachable devices
                    logger.error(f"❌ Device unreachable: {error_msg}")
                    if self._is_local_network:
                        logger.error("💡 Local network device should respond quickly - check IP address and power")
                    raise ConnectionError(f"Device unreachable: {error_msg}")
                else:
                    # Just log warning and continue with normal timeout behavior
                    logger.warning(f"⚠️ Quick check failed: {error_msg}")
                    logger.warning("🔄 Continuing with normal authentication (may take longer)")
        ...
        try:
            ...
            if login_response and any(term in login_response.lower() for term in ["success", "ok", "true"]):
                ...
                mode_str = "concurrent" if self.concurrent else "serial"
                device_type = "local" if self._is_local_network else "remote"
                logger.info(f"🎉 Authentication successful ({mode_str} mode, {device_type} device)! ({auth_time:.2f}s)")
            ...
            if self._is_local_network:
                logger.error("💡 Local network device connection failed:")
                logger.error("   1. Verify IP address is correct")
                logger.error("   2. Check that device is powered on")
                logger.error("   3. Ensure HTTPS service is enabled")
    ...
    def get_status(self) -> Dict[str, Any]:
        """
        Retrieve comprehensive modem status with HTTP compatibility, instrumentation,
        and built-in connectivity validation.

        Uses concurrent or serial requests based on configuration, with built-in
        HTTP compatibility handling for urllib3 parsing strictness and optional
        quick connectivity validation.
        """
        try:
            if not self.authenticated:
                if not self.authenticate():  # This now includes quick check
                    raise RuntimeError("Authentication failed")

            mode_str = "concurrent" if self.concurrent else "serial"
            device_type = "local" if self._is_local_network else "remote"
            logger.info(f"📊 Retrieving modem status with {mode_str} processing ({device_type} device)...")
            ...
            parsed_data['_device_type'] = 'local' if self._is_local_network else 'remote'
            parsed_data['_performance'] = {
                ...
                'quick_check_enabled': self.quick_check,
            ...
            raise ConnectionError(f"Failed to connect after {elapsed:.1f}s: {e}")
    ...
    def close(self) -> None:
        ...
        if self.capture_errors and self.error_captures:
            ...
            logger.info(f"📊 Session captured {total_errors} errors for analysis ({mode_str} mode, {device_type} device)")
            if compatibility_issues > 0:
                logger.debug(f"🔧 HTTP compatibility issues handled: {compatibility_issues}")
